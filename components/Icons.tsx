...
export function IconLinkedIn({ className = "", size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  const id = `linkedin-${useId().replace(/:/g, "-")}`;
  return (
...
export function IconGitHub({ className = "", size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  return (
    <svg className={`${sizeClass(size)} ${className}`} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
